...
func main() {
	...
	// 接收字符串，需要将字符串转换成结构体，然后操作 ==> 字符串 ==> 结构体 ==> 解码
	lily := Student{
		Id:     1,
		Name:   "Lily",
		Age:    20,
		gender: "female", // 注意: gender是小写的，小写字母开头的字段，在json编码时会忽略掉
	}

	// 编码（序列化），结构 => 字符串
	// func Marshal(v interface{}) ([]byte, error) {}
	encodeInfo, err := json.Marshal(&lily) // &lily是传递的地址
	if err != nil {
	...
	// 由于gender在结构体中是小写的，json编码时忽略掉，不会参与编码，所以这里打印的是空
